...
# Time-varying effect used only by the SC4/SC5 data generators
ia = function(t) {
  t
}
...
make_bias_validation = function(seed = bias_validation_seed) {
  set.seed(seed)
  data = t(sapply(1:nv, sim_data, censor = FALSE, shift = shift_v))
...
run_all_meth = function(rep, sc, bias_validation) {
  ...
  data_t = CoxTL_prepare_data(
    data[,-1,drop = FALSE], p, population = 0,
    covariates = c("X1", "X2")
  )
...
single_sc_sim = function(sc) {
  ...
  bias_output = paste0(
    "individual_survival_bias_single_source",
    if(shift_s) "_source_shift" else "",
    if(shift_v) "_validation_shift" else "",
    sprintf("_sc%d", sc)
  )
  save(individual_bias,
       file = paste0(bias_output, ".RData"))
  plot_individual_survival_bias(
    individual_bias,
    file = paste0(bias_output, ".pdf")
  )
  rm(sim_results)
...
res_by_sc = lapply(4:5, single_sc_sim)
