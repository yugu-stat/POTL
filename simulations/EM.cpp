  ...
  if(fabs(r)>1e-8) {
    for(int i=0; i<n; ++i) {
      ...
      for(int g=0; g<N; ++g) {
        ...
        numw(arma::span(0,l)) += gl.weight(g)*ewz(arma::span(0,l))*ru(g)/(1.0-exp(-ru(g)*sum0))*tmp;
      } // end g
      if(fabs(den) > 1e-8) {
        e.z(i) = numz/den;
        e.wbar(arma::span(i), arma::span(0,l)) = numw(arma::span(0,l)).t()/den;
      }
    } // end i
  } else {
    ...
    for(int i=0; i<n; ++i) {
      ...
      l--;
      e.wbar(arma::span(i), arma::span(0,l)) = ewz(arma::span(0,l)).t()/(1.0-exp(-sum0));
    }
  }
...
// [[Rcpp::export]]
mat IndividualSurvBias(const vec& beta,
                       const vec& lambda,
                       const vec& t,
                       const mat& rawXv,
                       double pred_r,
                       double true_r,
                       const vec& pred_times) {
  // Signed survival prediction errors for a fixed validation cohort.
  // pred_r defines the fitted survival transformation; true_r defines truth.
  // Predictions are truncated at the largest requested time so rawXv only
  // needs to cover the target prediction horizon.
  int nv, L, nt;
  vec fit_t, fit_lambda;
  uvec ind;
  
  if(t.n_elem != lambda.n_elem) {
    stop("t and lambda must have the same length.");
  }
  if(rawXv.n_rows==0) {
    stop("rawXv must contain at least one validation subject.");
  }
  if(rawXv.n_cols<4) {
    stop("rawXv must contain ID, interval, and covariate columns.");
  }
  if(!t.is_finite() || !t.is_sorted("ascend")) {
    stop("t must be finite and sorted in increasing order.");
  }
  
  nv = max(rawXv.col(0))+1;
  nt = pred_times.n_elem;
  if(nt==0) {
    return mat(nv, 0);
  }
  if(!pred_times.is_finite() || any(pred_times<=0) ||
     !pred_times.is_sorted("ascend")) {
    stop("pred_times must be finite, positive, and sorted in increasing order.");
  }
  
  ind = find(t<=max(pred_times));
  L = ind.n_elem;
  fit_t = t.head(L);
  fit_lambda = lambda.head(L);
  
  return individualSurvBias(pred_times, fit_t, fit_lambda, beta, rawXv,
                            pred_r, true_r, nv);
}
...
List TransFit(const vec& Y,
  ...
  // By default, preserve the original behavior. Supplying true_r allows
  // predictions from a fitted transformation r to be evaluated against a
  // different data-generating transformation.
  if(true_r < 0.0) {
    true_r = r;
  }
  ...
  res["Stau"] = Stau(t, pred, Xv, true_r, nv, Lv, endt);
  
  // supremum distance between Shat and true S
  res["supdistS"] = supdistS(t, pred, Xv, true_r, nv, Lv, endt);
  
  // L2 distance between Shat and true S
  res["L2distS"] = L2distS(t, pred, Xv, true_r, nv, Lv, endt);
