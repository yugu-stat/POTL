...
mat individualSurvBias(const vec& pred_times,
                       const vec& t,
                       const vec& lambda,
                       const vec& beta,
                       const mat& rawXv,
                       ...
                       double true_r,
                       int nv) {
  // Compute Shat(u|X_i)-S(u|X_i) for every validation subject and
  // prediction time. The mean across simulation replications is
  // calculated in R while retaining the distribution across subjects.
  double Shat, ebx, sum0, u;
  int raw_row, row_end, fit_row, grid_row, l;
  int L = t.n_elem;
  int nt = pred_times.n_elem;
  int p = beta.n_elem;
  int ptrue = rawXv.n_cols-3;
  mat bias(nv, nt);
  vec xtrue(ptrue);
  
  if(p>ptrue) {
    stop("beta contains more covariates than rawXv.");
  }
  
  raw_row = 0;
  for(int i=0; i<nv; ++i) {
    if(raw_row>=rawXv.n_rows || rawXv(raw_row,0)!=i) {
      stop("rawXv IDs must be zero-based, contiguous, and sorted.");
    }
    row_end = raw_row;
    while(row_end<rawXv.n_rows && rawXv(row_end,0)==i) {
      row_end++;
    }
    
    fit_row = grid_row = raw_row;
    l = 0;
    sum0 = 0.0;
    for(int j=0; j<nt; ++j) {
      u = pred_times(j);
      while(l<L && t(l)<=u) {
        while(fit_row<row_end &&
              !(rawXv(fit_row,1)<t(l) && t(l)<=rawXv(fit_row,2))) {
          fit_row++;
        }
        if(fit_row==row_end) {
          stop("rawXv does not cover all fitted event times.");
        }
        ebx = exp(as_scalar(beta.t()*rawXv(fit_row, arma::span(3,p+2)).t()));
        sum0 += lambda(l)*ebx;
        l++;
      }
      Shat = StX(sum0, pred_r);
      
      while(grid_row<row_end &&
            !(rawXv(grid_row,1)<u && u<=rawXv(grid_row,2))) {
        grid_row++;
      }
      if(grid_row==row_end) {
        stop("rawXv does not cover all prediction times.");
      }
      xtrue = rawXv(grid_row, arma::span(3,ptrue+2)).t();
      bias(i,j) = Shat-trueS(u, xtrue, true_r);
    }
    raw_row = row_end;
  }
  if(raw_row!=rawXv.n_rows) {
    stop("rawXv IDs must be grouped in increasing order.");
  }
  return bias;
}
